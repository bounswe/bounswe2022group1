...
def get_all_courses(req):
   ...
   query = f"SELECT COUNT(1) FROM {dbname}.users WHERE username = '{username}' AND is_teacher = 0"
   result = run_statement(query)[0][0]
   if result == 0:
      return JsonResponse({"error": "Student not found!"}, status=404)

   # SQL query
   query = f"SELECT course_name FROM {dbname}.enrolls WHERE student_username = '{username}'"
...
def student_my_courses_seen_update(req):
   ...
   # Update
   query = f"UPDATE {dbname}.users SET last_course_view_time = {time.time()} WHERE username = '{username}';"
   run_statement(query)
